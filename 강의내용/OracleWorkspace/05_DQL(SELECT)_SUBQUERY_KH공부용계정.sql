...
-- 전체사원의 평균 급여보다 더 많은 급여를 받고 있는 사원들의 사번, 이름, 직급코드
...
SELECT MAX(SUM(SALARY)) -- 가장 큰 값 구하기
FROM EMPLOYEE
GROUP BY DEPT_CODE;
...
GROUP BY DEPT_CODE, DEPT_TITLE -- 부서별 분류
...
-- 2) ANY
-- 대리직급임에도 불구하고 과장 직급의 급여보다 많이 받는 사원들 조회
-- 사번, 이름, 직급명, 급여
...
/*
    3. (단일행) 다중열 서브쿼리
     - 서브쿼리 조회 결과가 한행이지만, 카럼의 갓수가 2개 이상인 경우
*/
...
-- 소속은 부서가 없는 경우 NULL처리함수를 통해 부서없음으로 조회하시오.
SELECT MAX(SALARY)
FROM EMPLOYEE
GROUP BY DEPT_CODE;
...
WHERE (NVL(DEPT_CODE, '부서없음'), SALARY)
    IN (SELECT NVL(DEPT_CODE, '부서없음'), MAX(SALARY)
        FROM EMPLOYEE
        GROUP BY DEPT_CODE);

/*
    5. 스칼라 서브쿼리
     - 단일행 단일열 서브쿼리의 일정으로 SELECT절에서 사용되는 서브쿼리를 의미한다.
     - SELECT절이 실행될 때마다 서브쿼리문이 실행되면서 조회결과값을 반환한다.
     - 현재 조회된 행의 칼럼값을 서브쿼리내에서 기술이 가능하며, 기술하는 경우 연관쿼리라고
       부른다.
     - 매 행마다 실행되기 때문에 대규모 데이터 처리시 효율이 좋지 못하다(연관쿼리)
*/
-- 직원번호, 직원명, 부서명 조회(단, JOIN 없이)
SELECT EMP_ID, EMP_NAME,
    (SELECT DEPT_TITLE FROM DEPARTMENT WHERE E.DEPT_CODE = DEPT_ID ) DEPT_TITLE
-- 마지막에 있는 DEPT_TITLE은 별칭으로 없어도 동작한다.
FROM EMPLOYEE E;
...
-- *ROWNUM : 오라클에서 제공해주는 칼럼
-- 조회된 수서대로 1부터 겹치지 않는 순번을 부여해주는 칼럼
