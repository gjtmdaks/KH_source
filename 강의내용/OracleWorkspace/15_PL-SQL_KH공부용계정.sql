/*
    <PL/SQL>
     - FROCEDURE LANGUAGE EXTENSTION TO SQL
...
END;
/
-- 4) CASE 비교대사 WHEN 동등비교값 THEN 결과값 WHEN 비교값 THEN 결과값 ... END
...
DECLARE
    Y NUMBER;
BEGIN
    FOR EMP IN (SELECT EMP_NAME, SALARY, BONUS FROM EMPLOYEE)
    LOOP
        IF EMP.BONUS IS NULL THEN DBMS_OUTPUT.PUT_LINE(EMP.EMP_NAME || ' 사원의 연봉은 ' || EMP.SALARY*12 || '입니다.');
        END IF;
        IF EMP.SALARY IS NULL THEN DBMS_OUTPUT.PUT_LINE(EMP.EMP_NAME || ' 사원은 연봉이 지정되지 않았습니다.');
        ELSE DBMS_OUTPUT.PUT_LINE(EMP.EMP_NAME || ' 사원의 연봉은 ' || (EMP.SALARY+EMP.SALARY*EMP.BONUS)*12 || '입니다.');
        END IF;
    END LOOP;
END;
/
