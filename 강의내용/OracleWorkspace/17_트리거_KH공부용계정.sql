/*
    <트리거>
     ...
     - EX) 회원탈퇴시 기존의 회원테이블에서 데이터를 삭제(DELETE) 후, 곧바로 탈퇴된 회원들만
       ...
       신고횟수가 일정수를 넘었을때 해당 회원의 정보를 즉시 변경하는 경우.
    ...
    * 트리거 종류
    ...
    2. AFTER TRIGGER
     - 내가 트리거로 지정한 테이블에 DML이 발생된 "후"에 트리거를 실행
...
CREATE OR REPLACE TRIGGER AFTER1
...
/*
    트리거 장점
    1. 데이터 추가, 수정, 삭제시 자동으로 데이터괄리를 해줌으로써 데이터베이스의 무결성보장.
    2. 데이터베이스의 관리 자동화
    
    트리거 단점
    1. 빈번한 추가, 수정, 삭제 시 다른 테이블의 행 또한 추가로 추가, 수정, 삭제가 되므로
       성능이 좋지 못하다.
