/*
    ...
    5. [WHERE 조건식]
    3. START WITH 시작조건
    4. CONNECT BY [PRIOR] 상위값 = 하위값
    7. [ORDER SIBLINGS BY 정렬조건]
    
    순서를 바꾸고 싶다면 인라인 뷰로 FROM 테이블을 만들고 참조한다.
*/

-- 계층형 쿼리 샘플 스크립트 시작--
-- 1개의 테이블에서 데이터를 관리하는경우 (댓글)
CREATE TABLE comments ( -- COMMENT는 예약어라 테이블명으로 사용불가
...
COMMENT ON COLUMN comments.user_name IS '작성자';
COMMENT ON COLUMN comments.content IS '댓글 내용';
COMMENT ON COLUMN comments.created_at IS '작성일자';
...
SELECT LEVEL, LPAD(' ',(LEVEL-1)*2) || USER_NAME 작성자, COMMENT_ID, PARENT_ID, CONTENT, CREATED_AT
FROM COMMENTS
-- 계층구조의 시작점을 정의
START WITH PARENT_ID IS NULL
-- 부모-자식관계를 정의
--          부모의 ID(상위값) = 자식의 ID(하위값)
...
SELECT LPAD(' ',(LEVEL-1)*2) || USER_NAME 작성자, CONTENT, CREATED_AT
FROM COMMENTS
START WITH USER_NAME = '민경민' AND PARENT_ID IS NULL
CONNECT BY PRIOR COMMENT_ID = PARENT_ID
ORDER SIBLINGS BY CREATED_AT DESC;

-- EMPLOYEE테이블에서 선동일 사원과 선동일 사원이 사수인 사원들을 조회
-- LEVEL, 사번, 사원이름, 매니저 ID
SELECT LEVEL, EMP_ID, LPAD(' ',(LEVEL-1)*2) || EMP_NAME, MANAGER_ID
FROM EMPLOYEE

-- 선동일과 그 직속 후배만 조회하고 싶다면
--WHERE EMP_NAME = '선동일' OR
--  MANAGER_ID = (SELECT EMP_ID FROM EMPLOYEE WHERE EMP_NAME='선동일')

START WITH EMP_NAME = '선동일'
