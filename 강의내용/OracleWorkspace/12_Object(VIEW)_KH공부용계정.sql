...
SELECT *
FROM VW_EMPLOYEE
WHERE BONUS IS NULL; -- VIEW에 존재하지 않는 칼럼 사용시 오류발생
...
AS SELECT EMP_ID, EMP_NAME, JOB_NAME, SUBSTR(EMP_NO,8,1) 성별, 
        EXTRACT(YEAR FROM SYSDATE) - EXTRACT(YEAR FROM HIRE_DATE) 근무년수
    FROM EMPLOYEE
    JOIN JOB USING(JOB_CODE);
-- must name this expression with a column alias
-- 컬럼에 함수를 사용한 경우 별칭부여가 필수

SELECT * FROM VW_EMP_JOB;

-- 또 다른 별칭부여 방법(칼럼 전부에게 부여해야만 함)
...
/*
    ...
    * 뷰를 통해 DML이 불가능한 경우
    ...
    4) 그룹함수나 GROOUP BY절이 포함된 경우
    5) DISTINCT구문이 포함된 경우
    6) JOIN을 통해 여러 테이블을 매칭시켜놓은 경우
*/
-- 1) 뷰에 정의되지 않은 칼럼을 조작
-- 2) 뷰에 정의되어 있지 않은 칼럼 중, 원본 테이블상에 NOT NULL제약조건이 추가된 경우.
CREATE OR REPLACE VIEW VW_JOB
AS SELECT JOB_NAME FROM JOB;

INSERT INTO VW_JOB(JOB_CODE, JOB_NAME) VALUES('J9','인턴'); -- 실패
-- 하나만 넣으면 JOB_CODE cannot insert NULL
-- 두개를 넣으면 too many values
-- 앞에서 지정하여 넣으면 "JOB_CODE": invalid identifier -> 뷰에 JOB_CODE가 없다

UPDATE VW_JOB
SET JOB_NAME = '인턴'; -- 성공
-- JOB_CODE를 아예 안쓰면 가능함(단, 모든 행이 다 변경됨)
ROLLBACK;
...
UPDATE VW_JOINEMP
SET DEPT_TITLE = '회계부'
WHERE EMP_ID = 200; -- 두개의 테이블을 사용했기에 실패
...
-- 3. WITH CHECK OPTION
-- SELECT문의 WHERE절에서 사용한 칼럼을 수정하지 못하게 막는 옵션.
...
-- 물론 나머지 컬럼은 얼마든지 변경 가능

UPDATE V_CHECKOPTION
SET SALARY = 6000000
WHERE EMP_ID = 206;
-- WHERE절에 기술한 칼럼이 아니므로 변경 가능.
...
UPDATE V_READ
SET SALARY = 1000000000000000000;
-- cannot perform a DML operation on a read-only view
-- READ-ONLY뷰에서는 DML사용이 불가능하다.
