/*
    <함수 FUNCTION>
     ...
     - 단일행함수 :
            N개의 값을 읽어서 N개의 결과를 리턴하는 함수.(매행마다 함수 실행 후 결과 반환)
     - 그룹함수 :
            N개의 값을 읽어서 N개(기본값 1개)의 결과를 리턴하는 함수(그룹별 함수 실행 후 결과 반환)
...
/*
    <문자열과 관련된 함수>
    LENGTH(글자수) / LENGTHB(글자 바이트 수)(한글은 3바이트)
...
/*
    INSTR
     ...
     - INSETR(문자열, 찾을문자, 찾을위치의 시작값, 순번)
*/
SELECT INSTR('AABAACAABBAA', 'B')
FROM DUAL;

SELECT INSTR('AABAACAABBAA', 'B', -1)
FROM DUAL; -- 시작값에 -1 넣을 시 뒤에서부터 문자를 찾는다.
...
/*
    LTRIM/RTRIM
     ...
     - 문자열 기준 왼쪽/오른쪽에서부터 제거시키고자 하는 문자들을 찾아서 제거한 나머지를 반환
     - 제거할 문자 생략시 기본값은 ' '.
*/
SELECT LTRIM('    K   H   ')
FROM DUAL;

SELECT LTRIM('0000120304056000','0')
FROM DUAL;

SELECT RTRIM('0000120304056000','0')
FROM DUAL;
...
SELECT LTRIM(RTRIM('1385423687KH128233','1234567890'), '1234567890')
FROM DUAL;

/*
    TRIM
     - TRIM(BOTH/LEADING/TRAILING '제거할문자' FROM '문자')
     - 옵션과, 제거할 문자 생략시 기본값으로 BOTH, ' '이 선택된다.
*/
SELECT TRIM('    K   H    ')
FROM DUAL;

SELECT TRIM('Z' FROM 'ZZZKHZZZ') "Z삭제"
FROM DUAL;
...
SELECT LOWER('Welcome to G class'),
        ...
        INITCAP('Welcome to G class') -- 각 단어의 첫글자를 대문자로 변경
FROM DUAL;
...
*/
SELECT REPLACE('서울시 강남구 역삼동','역삼동','상봉동')
FROM DUAL;
...
SELECT '20'||SUBSTR(SYSDATE, 1,2)||'년' 연도, SUBSTR(SYSDATE, 4,2)||'월' 월, SUBSTR(SYSDATE, 7,2)||'일' 일
FROM DUAL;
...
FROM DUAL; -- 1234 -> '01234' 빈칸을 '0'으로 채운다

SELECT TO_CHAR(1234, '99999')
FROM DUAL; -- 1234 -> '1234' 빈칸을 ' '으로 채운다

SELECT TO_CHAR(1234, 'L99999')
FROM DUAL; -- L : LOCAL -> KOREAN = ￦

SELECT TO_CHAR(1234, '99,999')
FROM DUAL; -- '1,234' 3자리마다 , 삽입
...
FROM DUAL; -- DATE -> STRING

SELECT TO_CHAR(SYSDATE, 'YYYY-MM-DD')
FROM DUAL;

-- 시 분 초
SELECT TO_CHAR(SYSDATE, 'HH24:MI:SS') -- HH뒤에 12시간/24시간 기준 설정
FROM DUAL;

-- 요일
SELECT TO_CHAR(SYSDATE, 'MON DD DY DAY')
FROM DUAL;
...
-- 사실상 204와 동일 => 자리수 불일치로 오류발생
-- 문자열 '000204'로 하면 문제 없음
...
FROM DUAL; -- YY는 앞에 무조건 20을 붙여서 2098년으로 포맷된다

SELECT TO_DATE('980630','RRMMDD')
FROM DUAL; -- 따라서 1998년으로 하길 원하면 RR을 사용해야 한다.
...
SELECT TO_NUMBER('10,000','99999') + TO_NUMBER('50,000','99,999')
FROM DUAL;
...
/*
    <NULL처리 함수>
     ...
     - NVL은 칼럼에 값이 존재하는 경우 컬럼값을, 존재하지 않는 경우 대체값을 반환
*/
-- 사원명, 보너스, 보너스가 없는경우 0으로 출력
...
/*
    <선택함수>
     ...
     일치하는 값이 없을때 마지막 결과값을 안적으면 NULL반환
*/
SELECT DECODE('123','1234','일치1','123','일치2')
FROM DUAL;

-- 사번, 사원명, 주민등록번호, 성별
SELECT EMP_ID, EMP_NAME, EMP_NO,
    DECODE(SUBSTR(EMP_NO,8,1),'1','남자','2','여자','3','남자','4','여자') "성별(문자열)",
    DECODE(SUBSTR(EMP_NO,8,1),1,'남자',2,'여자',3,'남자',4,'여자') "성별(숫자)"
FROM EMPLOYEE;

/*
    CASE WHEN THEN 구문
     - DECODE는 조건검사시 동등비교만을 수행한다면,
       CASE WHEN THEN구문은 내 마음대로 조건을 설정할 수 있다.
...
/*
    그룹함수
     - 그룹함수는 데이터들의 합, 평균 등을 구할 때 사용한다.
     - N개의 값을 읽어서 그룹의 개수만큼의 결과를 반환한다.
*/
-- 1. SUM(숫자타입칼럼) : 해당 칼럼들의 총 합계를 반환해주는 함수
-- 전체 사원들의 총 급여 합
SELECT SUM(SALARY)
FROM EMPLOYEE
--WHERE 1=0
...
-- COUNT(*/컬럼이름/DISTINCT 칼럼)
-- 조회된 행의 갯수를 세서 반환하는 함수
-- COUNT(*) : 조회결과에 해당하는 모든 행의 갯수를세서 반환
-- COUNT(칼럼) : 조회결과중 칼럼값이 NULL이 아닌 행만 세서 반환
-- COUNT(DISTINCT 칼럼) : 제시한 칼럼값에 중복이 있을 경우 1개로 세서 반환.(NULL 미포함)
SELECT *
FROM EMPLOYEE;
...
/*
    ...
    RANK() OVER(그룹화기준 정렬기준)
     - 공동순위가 존재하는 경우 공동순위의 다음순위는 공동순위+공동순위의 갯수만큼 뒤로해서 부여
     - EX) 공동 1위가 3명인 경우 다음 순위는 4위
    
    DENSE_RANK() OVER(그룹화기준 정렬기준)
     - 공동1위가 3명이라고 해도, 그 다음순위는 무조건 2위로 처리하는 방식
...
SELECT EMP_NAME, SALARY, DEPT_CODE,
    ROW_NUMBER() OVER(PARTITION BY DEPT_CODE ORDER BY SALARY DESC) "순위"
FROM EMPLOYEE;
...
-- 조회결과내에서 월급기준 내림차순 정렬 수행 후, 현재행 기준 이전 행의 월급 가져오기.
SELECT EMP_NAME, DEPT_CODE, SALARY, LAG(SALARY) OVER(ORDER BY SALARY DESC) "이전직원월급"
FROM EMPLOYEE;

SELECT EMP_NAME, DEPT_CODE, SALARY, LEAD(SALARY) OVER(ORDER BY SALARY DESC) "이후직원월급"
FROM EMPLOYEE;

SELECT EMP_NAME, DEPT_CODE, SALARY, LAG(SALARY, 2, 0) OVER(ORDER BY SALARY DESC) "2이전직원월급"
FROM EMPLOYEE;

-- 실습 --
-- 직원의 사원번호, 이름, 직급코드, 월급을 조회한 후
-- 조회결과내에서 직급코드별로 그룹화 하여, 소그룹내에서 전사람의 월급을 가져오시오
-- 단, 가져올 데이터가 없는 경우, 0원으로 표시하며
-- 소그룹내에서는 사원번호 기준 오름차순 정렬하시오.
SELECT EMP_ID, EMP_NAME, JOB_CODE, SALARY, 
        LAG(SALARY,1,0) OVER(PARTITION BY JOB_CODE ORDER BY EMP_ID) "이전직원월급"
FROM EMPLOYEE;
