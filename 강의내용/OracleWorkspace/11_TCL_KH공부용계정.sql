/*
    TCL(TRANSACTION CONTROL LANGUAGE)
     ...
     - COMMIT : 하나의 트랜잭션에 담겨있는 변경사항들을 실제 DB에 반영하는 명령어
                COMMIT 실행 후, 트랜잭션 내부의 변경사항은 모두 비워진다.(CLEAN)
     ...
     - ROLLBACK TO 포인트명 : 전체 변경사항을 취소하는게 아니라, 포인트 지정까지의
                             트랜잭션만 롤백하는 명령어
*/
/*
...
*/
-- 제약조건이 추가된 테이블에 제약조건을 준수한데이터, 준수하지 않은 데이터를 추가
ALTER TABLE EMP_01
...
/*
    3. Isolation(격리성)
     ...
     - 만약 하나의 테이블에 대해 동시에 여러 세션에서 DML이 수행되는 경우, 내가 테이블의
       중간상태를 보고 작성한 코드가 제대로 실행되지 않을 수 있다. 이를 방지하기 위해서
       RDBMS는 트랜잭션의 DML이 완전히 COMMIT, ROLLBACK되기 전까지는 다른 세션에서 DML을
       수행하지 못하도록 LOCK을 건다.
...
CREATE TABLE TEST(
    TID NUMBER
); -- 테이블 생성 직전에 COMMIT 자동 실행
-- 사원수 0명 고정

ROLLBACK;
SELECT * FROM EMP_01; -- 사원수 0
