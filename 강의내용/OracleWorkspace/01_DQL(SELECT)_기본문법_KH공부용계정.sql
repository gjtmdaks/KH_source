...
select emp_id, emp_name, salary
from employee; -- 대문자로 작성하는 것이 권장된다.
...
*/
-- 연봉 별칭 부여
SELECT EMP_NAME AS 사원명, SALARY AS "급여(월)", SALARY*12 "연봉(보너스 미포함)"
FROM EMPLOYEE;
...
/*
    <WHERE절>
     - 조회하고자 하는 테이블에 특정 조건을 제시해서 그 조거넹 맞는 데이터들만 조회할 수 있다.
    [표현법]
    SELECT 조회할 칼럼명, ...
    FROM 테이블명
    WHERE 조건식;
...
-- 이메일 중 네번째 문자위치에 _가 있는 사원의 모든정보
SELECT *
FROM EMPLOYEE
...
/*
    <연결연산자 ||>
     ...
     - 칼럼과 리터럴도 연결이 가능하다.
     EX) System.out.print("num : " + num); 에서 + 역할
*/
...
SELECT *
FROM EMPLOYEE
ORDER BY SALARY DESC; -- 생략시 ASC가 기본값(오름차순)

-- 보너스 기준 정렬
-- * ORACLE에서는 정렬시 NULL값을 가장 큰값으로 생각한다.
SELECT *
FROM EMPLOYEE
--ORDER BY BONUS; -- ASC(오름차순) & NULLS FIRST가 기본값
