...
-- CNAME 칼럼을 새로 추가.
-- 칼럼추가 (ADD) : ADD 컬럼명 자료형 [DEFAULT]
ALTER TABLE DEPT_COPY
ADD CNAME VARCHAR2(20);

-- LNAME 칼럼을 DEPT_COPY에 추가하고 기본값으로 '한국'을 부여.
ALTER TABLE DEPT_COPY
...
ALTER TABLE DEPT_COPY
MODIFY DEPT_ID CHAR(3); -- CHAR(2) -> CHAR(3)

-- DEPT_ID칼럼을 NUMBER로 변경
ALTER TABLE DEPT_COPY
MODIFY DEPT_ID NUMBER; -- CHAR(3) -> NUMBER
-- column to be modified must be empty to change datatype
-- 칼럼의 데이터타입을 변경하고 싶다면 비워놔라
-- 값이 이미 추가된 경우 변경이 불가능 -> 바꾸고자 한다면 DEPT_ID의 모든 값을
-- NULL로 변경해야한다.

-- DEPT_ID칼러므이 값을 CHAR(2)로 변경
ALTER TABLE DEPT_COPY
MODIFY DEPT_ID CHAR(2); -- CHAR(3) -> CHAR(2)
-- 빈칸 포함해서 3바이트의 크기로 고정되어 버림
-- cannot decrease column length because some value is too big
-- XHAR(2)보다 더 큰 값이 저장된 경우 변경 불가능
...
ALTER TABLE DEPT_COPY2 DROP COLUMN LNAME; -- 마지막 칼럼은 반드시 존재해야 한다.
-- cannot drop all columns in a table
-- 테이블 내의 모든 칼럼을 삭제할 수는 없다.
...
-- DEPT_TITLE에 UNIQUE, LNAME에 NOT NULL 제약조건 추가
ALTER TABLE DEPT_COPY
...
ALTER TABLE DEPT_COPY
RENAME CONSTRAINT SYS_C007171 TO DCOPY_DN_UQ;
