/*
    <JOIN>
     ...
     - 관계형데이터베이스(RDBMS)에서는 데이터의 중복을 피하기 위해, 다양한 테이블에
       데이터를 나눠서 보관하고 있으며, 이 때 여러 테이블의 데이터를 동시에 조회하기
       위해서는 각 테이블이 가지고 있는 외래키를 활용하여 JOIN시켜줘야 한다.
       
    JOIN문은 "오라클 전용구문"과 "ANSI구문"으로 나뉘어져 있다.
    ...
    ------------------------------------------------------
        자체조인(SELF JOIN)
        비등기조인(NON EQUAL JOIN)
*/

/*
    1. 등가조인(EQUAL JOIN) / 내부조인(INNER JOIN)
     - 연결시키고자 하는 칼럼의 값이 "일치"하는
    ...
    SELECT 칼럼명들
    FROM 테이블1
    JOIN 조인할 테이블2 [ON/USING] 연결할 칼럼에 대한 조건을 제시
*/
...
SELECT EMP_ID, EMP_NAME, E.DEPT_CODE, DEPT_TITLE
...
-- 자연조인(NATURAL JOIN)
-- 등기조인 중 하나로, 동일한 타입과 칼럼명을 가진 칼럼을 조인조건으로 이용
-- 하는 조인문
SELECT EMP_ID, EMP_NAME, JOB_CODE, JOB_NAME
FROM EMPLOYEE
NATURAL JOIN JOB;
...
FROM EMPLOYEE, DEPARTMENT
WHERE DEPT_CODE = DEPT_ID(+);
-- 기준으로 삼을 테이블의 반대 테이블의 칼럼명에 (+)를 붙여준다.
